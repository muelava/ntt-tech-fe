...
export default function ProductCard({ product, onEdit, onDelete }: ProductCardProps) {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden hover:shadow-md transition-shadow">
      ...
      <div className="p-4">
        <p className="text-xs text-violet-600 font-medium uppercase mb-1">{product.category}</p>
        ...
        <div className="grid grid-cols-3 md:grid-cols-2 gap-2">
          ...
          <Button variant="danger" icon={Trash2} className="flex-1 text-sm" onClick={() => onDelete(product.id)}>
            Hapus
          </Button>
          <Button variant="secondary" icon={Eye} className="text-sm md:col-span-2" onClick={() => navigate(`/products/${product.id}`)}>
            Detail
          </Button>
        </div>
      </div>
